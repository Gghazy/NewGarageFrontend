...
interface ExteriorBodyStageResultDto {
...
export class ExteriorBodyStageComponent extends BaseStageComponent implements OnInit, OnDestroy {
  ...
  save(): void {
    ...
    this.api.post<ApiResponse<string>>(`Examinations/${examId}/stages/exterior-body`, payload)
      .pipe(
  ...
  private loadData(): void {
    ...
    this.loading = true;
    forkJoin({
      parts: this.api.post<any>('ExteriorBodyParts/pagination', search),
      issues: this.api.post<any>('ExteriorBodyIssues/pagination', search),
    }).pipe(
      tap(res => {
      ...
      switchMap(() => {
        ...
        return this.api.get<ApiResponse<ExteriorBodyStageResultDto>>(`Examinations/${examId}/stages/exterior-body`);
      }),
  ...
  private applyExistingData(data: ExteriorBodyStageResultDto | null): void {
    if (!data) return;
