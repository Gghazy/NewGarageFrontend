...
interface InteriorDecorStageResultDto {
...
export class InteriorTrimStageComponent extends BaseStageComponent implements OnInit, OnDestroy {
  ...
  save(): void {
    ...
    this.api.post<ApiResponse<string>>(`Examinations/${examId}/stages/interior-decor`, payload)
      .pipe(
  ...
  private loadData(): void {
    ...
    this.loading = true;
    forkJoin({
      parts: this.api.post<any>('InsideAndDecorParts/pagination', search),
      issues: this.api.post<any>('InsideAndDecorPartIssues/pagination', search),
    }).pipe(
      tap(res => {
      ...
      switchMap(() => {
        ...
        return this.api.get<ApiResponse<InteriorDecorStageResultDto>>(`Examinations/${examId}/stages/interior-decor`);
      }),
  ...
  private applyExistingData(data: InteriorDecorStageResultDto | null): void {
    if (!data) return;
